# from spektral_me.data import Graph,Dataset
from spektral_me.data_me import Graph,Dataset
...
class Graphmodel():
    def __init__(self,data,hop,distance,sitetype,feature_chose) -> None:
        """
        feature_chose: 1:Onehot, 2:Word2Vec, 3:the combination of Onehot and Word2Vec
        """
        self.data=data
        self.hop=hop
    ...
    def evaluate(self,loader,ite,model):
        ...
        while step < loader.steps_per_epoch:
            ...
            if step == loader.steps_per_epoch:
                os.makedirs(os.path.join('../Result/BO',self.__class__.__name__,'middle_output'), exist_ok=True)
                if True:
                    os.makedirs(os.path.join('../Result/BO',self.__class__.__name__,'middle_output'), exist_ok=True)
                    os.makedirs(os.path.join('../Result/BO',self.__class__.__name__,'middle_output'), exist_ok=True)
                    with open (os.path.join('../Result/BO',self.__class__.__name__,'middle_output','evaluate_output.txt'),'a') as f:
                        f.write(str(self.metrics_out(true_label,pre_label,pre_score))+'\n')
                    np.save(os.path.join('../Result/BO',self.__class__.__name__,'middle_output','test_BO.npy'),pre_score)
                    np.save(os.path.join('../Result/BO',self.__class__.__name__,'middle_output','true_BO.npy'),true_label)
